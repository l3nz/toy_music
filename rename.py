...
class Rename:
    ...
    def find_non_uniques(self):
        ...
        """
        risultato = []
        lista_ordinata = sorted(self.src, key=lambda x: self.remove_numeric_prefix(x[1]))
    ...
    def process_files(self):
        ...
        for (d, f) in self.src:
            if self.get_numeric_prefix(f) < 0:
                results.append( (self.ACTION_RENAME, d, f, f"{self.to_num(n)}-{f}"))
                n = n+ 1
        return results
    ...
    def to_num(n: int) -> str:
        ...
        """
        if n < 0 or n > 9999:
            raise ValueError("Il numero deve essere compreso tra 0 e 9999 per la formattazione a 4 cifre.")
